...
def load_or_train_pipeline():
    # If a pre-saved joblib model exists, load it
    for path in ["osteoporosis_pipeline.joblib", "model.joblib", "pipeline.pkl"]:
    ...
    # Fallback: Train the optimal tuned pipeline using osteoporosis.csv
    csv_file = "osteoporosis.csv"
    if not os.path.exists(csv_file):
        st.error("Dataset 'osteoporosis.csv' not found. Please ensure the dataset exists.")
        st.stop()
